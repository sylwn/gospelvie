...
jQuery(document).ready(function(){
	...
	// =============================================================================== TOGGLE THE ADD NEW MEGAMENU PART


	jQuery('.cro_toggle_me').click(function() {
		jQuery('.cromatic_megamenu_creator').slideToggle('slow');
	});
	...
	// ============================================================================ MEGAMENU DRAGGABLE
	 ...
	 jQuery( "ul.cro_megamenu li" ).draggable({
	 	helper: "clone"
	 });
	 ...
	 // ============================================================================ MEGAMENU DROPPABLE
	 jQuery('.cromatic_page_dropper ul').droppable({
	 	 tolerance: "intersect",
	 	 drop: function( event, ui ) {
	 	 	var clone 		= ui.draggable.clone(),
	 	 		info 		= clone.find('.bandesc').data('cro-data'),
	 	 		side 		= jQuery(this).data('cro-side'),
	 	 		linkholder 	= jQuery('.cro_mm_new_link'),
	 	 		newtitle 	= jQuery('.create_mm_text'),
	 	 		sidestr		= (side == 'l')? 'cro-leftside' : 'cro-rightside' ;

	 	 		
	 	 	linkholder.data(sidestr,info);
	 	 		

	 	 	var linkurl		= linkholder.data('cro-mm-url'),
	 	 		linkone		= linkholder.data('cro-leftside'),
	 	 		linktwo		= linkholder.data('cro-rightside'),
	 	 		linktitle	= linkholder.data('cro-title'),
	 	 		newurl		= '';


	 	 	if (linkone != 0 && linktwo != 0 && linktitle != '' && linktitle != 0) {
	 	 		newurl = linkurl + '&leftside=' + linkone + '&rightside=' + linktwo + '&title=' + encodeURIComponent(linktitle);
	 	 		linkholder.attr('href',newurl);
	 	 		linkholder.show();
	 	 	}
	 ...
	 jQuery('.create_mm_text').on('input',function(e) {
	 	var linktitle	= jQuery(this),
	 		linkholder 	= jQuery('.cro_mm_new_link');

	 	linkholder.data('cro-title',linktitle.val());
