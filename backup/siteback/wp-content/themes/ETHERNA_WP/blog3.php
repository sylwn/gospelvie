...
?>
	...
	<div class="main">
		...
		<?php if($ds_eth_blog_alpha3){echo '<div class="container_alpha slogan">'.stripslashes($ds_eth_blog_alpha3).'</div>';}
		...
		<?php } ?>
			<div class="container_omega blogpage">
				<?php if($ds_eth_blog_sidebar_left3 != 0){ ?>
				<ul class="gs_<?php echo($ds_eth_blog_sidebar_left3); ?> sidebar_mirror">
					<?php if(function_exists(dynamic_sidebar(1))):
					dynamic_sidebar(1);
					endif; ?>
				</ul>
				<?php } ?>
				<div class="gs_<?php echo(12-$ds_eth_blog_sidebar_right3-$ds_eth_blog_sidebar_left3); ?> <?php if($ds_eth_blog_sidebar_right3 == 0){echo 'omega';} ?>">
					<?php $paged = 1;
					if ( get_query_var('paged') ) $paged = get_query_var('paged');
					if ( get_query_var('page') ) $paged = get_query_var('page');
					query_posts(array( 'category__in' => $ds_eth_categories_list3, 'paged' => $paged, 'posts_per_page' => $ds_eth_blog_ppp3)); ?>
					<?php if (have_posts()) : ?>
					<?php while (have_posts()) : the_post(); ?>
					<?php
					$picsize_x = ((((12-$ds_eth_blog_sidebar_right3-$ds_eth_blog_sidebar_left3)-1)*77)+33);
				...
				<?php if($ds_eth_blog_sidebar_right3 != 0){ ?>
				<ul class="gs_<?php echo($ds_eth_blog_sidebar_right3); ?> sidebar omega">
