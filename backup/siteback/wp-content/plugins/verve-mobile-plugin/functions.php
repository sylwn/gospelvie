<?php
function websitez_comscore_footer() {
	$verve_options = get_option('websitez-options');
	if(strlen($verve_options['general']['verve_partner_key']) == 0)
		return;
?>
<!--Begin mobile comScore Tag-->
<script type="text/JavaScript">var
_comscore=_comscore||[];_comscore.push({c1:"10",c2:"6036493"});(function(){
var 
src=(document.location.protocol=="https:"?"https://sb":"http://b")+".scorec
ardresearch.com/mobile.js",s=document.createElement("script"),el=document.g
etElementsByTagName("script")[0];if(el){s.src=src;s.async=true;el.parentNod
e.insertBefore(s,el)}else{document.write(unescape("%3Cscript
src='"+src+"'%3E%3C/script%3E"))}})();</script>
<noscript><div><img
src="http://b.scorecardresearch.com/p?c1=10&c2=6036493&c4=<?php echo urlencode($_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"]); ?>&c8=<?php echo urlencode(get_the_title()); ?>&c9=&Rn=<?php echo rand(1000,250000); ?>&cv=2.0&cj=1" alt="*"></div></noscript>
<!--End mobile comScore Tag-->
<?php
}
function websitez_omniture_footer() {
	$verve_options = get_option('websitez-options');
	if(strlen($verve_options['general']['verve_partner_key']) == 0)
		return;
	include_once(WEBSITEZ_PLUGIN_DIR."/omniture/wp-omniture.php");
}
...
function websitez_install(){
	global $wpdb, $verve_unlimited_version, $websitez_preinstalled_templates, $table_prefix;
	...
	*/
	if($verve_unlimited_version == true){
		/*
		...
		*/
		...
		$websitez_options = array(
			...
			'general' => array(
				"mobile_home_page"=>"posts",
				"selected_mobile_theme"=>"verve-mobile",
				"posts_per_page"=>"",
				"verve_partner_key"=>""
			),
			'analytics' => array(
			...
			'images' => array(
				"header_left_icon"=>"images/ico/1_blank.png",
				"custom_website_background"=>"images/bg.gif",
				"logo"=>""
			),
			'sidebar' => array(
				"menu_order"=>"show_search_div,show_menu_div,show_pages_div,show_categories_div,show_blogroll_div,show_meta_div",
				"show_menu"=>"yes",
				"show_pages"=>"yes",
				"show_pages_items"=>"",
				"show_categories"=>"yes",
		...
		if(!get_option("websitez-options"))
			add_option("websitez-options", $websitez_options, '', 'yes');
		else
		...
		if(!get_option(WEBSITEZ_SHOW_ATTRIBUTION_NAME))
			add_option(WEBSITEZ_SHOW_ATTRIBUTION_NAME, WEBSITEZ_SHOW_ATTRIBUTION, '', 'no');
		...
		if(strlen($authorization['message']) > 0)
			exit($authorization['message']);
		else
			exit("There was an error activating the plugin. Please try again.");
	}
}
...
function websitez_checkInstalled(){
	global $wpdb,$table_prefix,$verve_unlimited_version;
	$table = $table_prefix."options";
	if(!get_option(WEBSITEZ_BASIC_THEME) || !get_option(WEBSITEZ_ADVANCED_THEME)){
		if($verve_unlimited_version == true){
			add_action('admin_notices', create_function( '', "echo '<div class=\"error\"><p>".WEBSITEZ_PLUGIN_NAME." was unable to install correctly. Please try deactivating and then activating this plugin again.</p></div>';" ) );
		}else{
			add_action('admin_notices', create_function( '', "echo '<div class=\"error\"><p>".WEBSITEZ_PLUGIN_NAME." was unable to install correctly. This domain is not authorized to use this plugin.</p></div>';" ) );
		}
	}
}
...
function websitez_check_and_act_mobile(){
	global $table_prefix, $wpdb, $verve_unlimited_version, $websitez_preinstalled_templates;
	...
	if($mobile_device['status'] == true || $mobile_device['status'] == "1"){
		add_action('wp_footer', 'websitez_omniture_footer');
		add_action('wp_footer', 'websitez_comscore_footer');
...
function websitez_redirect(){
	...
	$page = $websitez_options['general']['mobile_home_page'];
	if($page && is_front_page() && is_numeric($page)){
...
function websitez_detect_mobile_device(){
  ...
  $user_agent = $_SERVER['HTTP_USER_AGENT'];
	...
	if(isset($_GET['verve-mobile'])){
		if(isset($_GET['verve-mobile-type'])){
			$type = $_GET['verve-mobile-type'];
		}else{
			$type = "1";
		}
